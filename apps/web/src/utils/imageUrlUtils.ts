...
export function normalizeImageUrl(url: string | null | undefined, fallback?: string): string {
  ...
  // Handle Cloudflare R2 keys that might not have the domain yet
  // Keys follow format: {uuid}/{folder}/{uuid}.ext
  const r2Domain = import.meta.env.VITE_R2_PUBLIC_DOMAIN;
  const isR2Key = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}\//.test(trimmed);
  
  if (isR2Key && r2Domain) {
    return `${r2Domain}/${trimmed}`;
  }
