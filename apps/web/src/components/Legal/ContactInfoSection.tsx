...
export const ContactInfoSection: React.FC<ContactInfoSectionProps> = ({
  ...
  const emailContact = contactInfo.find(c => c.contactType === 'email' && c.isPrimary);
  const phoneContact = contactInfo.find(c => c.contactType === 'phone' && c.isPrimary);
  const addressContact = contactInfo.find(c => c.contactType === 'address' && c.isPrimary);
  const whatsappContact = contactInfo.find(c => (c.contactType === 'whatsapp' || c.contactType === 'phone') && c.isPrimary);

  // Fallback to any contact if primary not found
  const email = emailContact?.value || contactInfo.find(c => c.contactType === 'email')?.value;
  const phone = phoneContact?.value || contactInfo.find(c => c.contactType === 'phone')?.value;
  const address = addressContact?.value || contactInfo.find(c => c.contactType === 'address')?.value;
