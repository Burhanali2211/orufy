...
export function transformProduct(product: any): Record<string, any> {
  if (!product) return product;

  const transformed = transformKeysToCamel<Record<string, any>>(product);
 ...
 */
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export function transformCategory(category: any): Record<string, any> {
  if (!category) return category;

  const transformed = transformKeysToCamel<Record<string, any>>(category);

  if (!transformed['imageUrl'] && category.image_url) {
