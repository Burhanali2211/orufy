    ...
    try {
      ...
      const options: any = {
        ...
        handler: async function (response: any) {
          try {
            const verifyRes = await fetch('/api/platform/payment/verify', {
              method: 'POST',
    ...
    >
      <div className="relative bg-gradient-to-br from-purple-600 via-purple-700 to-indigo-800 px-6 py-6">
        ...
        <div className="mt-6 text-center">
          ...
          <p className="text-4xl font-bold text-white mt-1">₹{(total / 100).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</p>
        </div>
      </div>
      ...
      <div className="p-6">
        <div className="bg-gray-50 rounded-2xl p-4 mb-6">
          <div className="space-y-2 text-sm">
            <div className="flex justify-between">
              ...
              <span className="font-medium">₹{(displaySubtotal / 100).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
            </div>
            {displayGst > 0 && (
              <div className="flex justify-between">
                <span className="text-gray-600">GST (18%)</span>
                <span className="font-medium">₹{(displayGst / 100).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
              </div>
            )}
            <div className="flex justify-between">
              <span className="text-gray-600">Shipping</span>
              <span className={`font-medium ${displayShipping === 0 ? 'text-green-600' : ''}`}>
                {displayShipping === 0 ? 'FREE' : `₹${(displayShipping / 100).toLocaleString('en-IN', { minimumFractionDigits: 2 })}`}
              </span>
            </div>
          </div>
        </div>
        ...
        >
          {isProcessing ? (
            <>
              <div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full hidden" />
              <span>Processing...</span>
            </>
          ) : (
            <>
              <Lock className="w-5 h-5" />
              <span>Pay ₹{(total / 100).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
