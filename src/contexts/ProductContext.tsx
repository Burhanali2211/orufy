...
export const ProductProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const fetchCategories = useCallback(async (background = false, force = false) => {
    ...
    try {
      const res = await Promise.reject(new Error("Mock fallback")) /* fetch('/api/categories') */;
      if (res.ok) {
      ...
      if (!cached) {
        setCategories([
          { id: '1', name: 'Category 1', slug: 'category-1', productCount: 4, sortOrder: 1, isActive: true },
          { id: '2', name: 'Category 2', slug: 'category-2', productCount: 2, sortOrder: 2, isActive: true },
          { id: '3', name: 'Category 3', slug: 'category-3', productCount: 3, sortOrder: 3, isActive: true },
        ]);
      }
    }
