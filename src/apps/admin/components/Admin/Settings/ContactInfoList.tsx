...
export const ContactInfoList: React.FC = () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) => apiClient.delete(`/admin/settings/contact/${id}`),
    onSuccess: () => {
  ...
  const toggleMutation = useMutation({
    mutationFn: (contact: ContactInfo) => 
      apiClient.put(`/admin/settings/contact/${contact.id}`, { is_active: !contact.is_active }),
    onSuccess: (_, variables) => {
      showSuccess(`Contact information ${!variables.is_active ? 'activated' : 'deactivated'}`);
      queryClient.invalidateQueries({ queryKey: ['admin-contact-info'] });
    },
    onError: (error: Error) => {
  ...
  const toggleActive = (contact: ContactInfo) => {
    toggleMutation.mutate(contact);
  };
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 sm:gap-4">
                    ...
                    <div className="flex items-center gap-1 sm:gap-2 flex-shrink-0">
                      <button
                        ...
                        className={`p-2 rounded-lg transition-colors min-h-11 min-w-[44px] sm:min-h-auto sm:min-w-auto flex items-center justify-center ${contact.is_active
                      ...
                      <button
                        ...
                        className="p-2 bg-purple-100 text-purple-700 rounded-lg hover:bg-purple-200 active:bg-purple-300 transition-colors min-h-11 min-w-[44px] sm:min-h-auto sm:min-w-auto flex items-center justify-center"
                      >
                      ...
                      <button
                        ...
                        className="p-2 bg-red-100 text-red-700 rounded-lg hover:bg-red-200 active:bg-red-300 transition-colors min-h-11 min-w-[44px] sm:min-h-auto sm:min-w-auto flex items-center justify-center"
                      >
