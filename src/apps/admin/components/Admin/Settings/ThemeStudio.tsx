...
export const ThemeStudio: React.FC = () => {
  ...
  return (
    <div className="space-y-4 max-w-[1600px] mx-auto pb-16">
      {/* ── Elementor Pro Top App Bar ── */}
      <div className="bg-white border border-stone-200 rounded-2xl px-5 py-3 shadow-xs flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[11px] text-stone-500 font-medium">Click any canvas section to edit properties in real-time.</p>
          </div>
        </div>
        ...
        {/* Right: Publish Live Action */}
        <div className="flex items-center gap-2.5">
          <button
            type="button"
            onClick={handlePublish}
            disabled={publishMutation.isPending}
            className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-stone-900 hover:bg-stone-800 text-white font-bold text-xs uppercase tracking-wider transition-all shadow-xs disabled:opacity-50 cursor-pointer"
          >
      ...
      {/* ── Main Workspace: Left Inspector Panel (380px) + Right Canvas ── */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        {/* ── Left Inspector Panel (Elementor Pro Inspector) ── */}
        <div className="lg:col-span-5 xl:col-span-4 bg-white border border-stone-200 rounded-2xl shadow-xs overflow-hidden flex flex-col min-h-[640px]">
          {/* Inspector Top Mode Selector */}
          {selectedSectionId ? (
            /* Mode 2: Selected Section Header */
          ...
          {/* Inspector Body */}
          <div className="p-5 flex-1 overflow-y-auto max-h-[700px] space-y-5">
            {/* ── FOCUSED BLOCK INSPECTOR (When a block is selected) ── */}
            {selectedSectionId === 'hero' && (
              <div className="space-y-5">
                  ...
                  <div className="space-y-4 pt-3 border-t border-stone-100">
                    <div className="flex items-center justify-between">
                      ...
                      <button
                        ...
                        className="px-2.5 py-1 rounded-lg bg-stone-900 text-white text-[11px] font-bold inline-flex items-center gap-1 cursor-pointer"
                      >
                    ...
                    <div className="flex items-center gap-1.5 overflow-x-auto pb-1">
                      {(hero.slides || []).map((_, idx) => (
                        <button
                          ...
                          className={`px-3 py-1.5 rounded-lg text-xs font-bold flex items-center gap-1.5 whitespace-nowrap cursor-pointer ${
                    ...
                    <div className="space-y-3 pt-2">
                      <div>
                        ...
                        <input
                          ...
                          className="w-full px-3 py-2 rounded-xl bg-stone-50 border border-stone-200 text-xs font-medium focus:bg-white focus:outline-none focus:ring-1 focus:ring-stone-900"
                        />
                      </div>
                      <div>
                        ...
                        <input
                          ...
                          className="w-full px-3 py-2 rounded-xl bg-stone-50 border border-stone-200 text-xs font-bold focus:bg-white focus:outline-none focus:ring-1 focus:ring-stone-900"
                        />
                      </div>
                      <div>
                        ...
                        <textarea
                          ...
                          className="w-full px-3 py-2 rounded-xl bg-stone-50 border border-stone-200 text-xs font-medium focus:bg-white focus:outline-none focus:ring-1 focus:ring-stone-900 resize-none"
                        />
                      </div>
                      <div>
                        <label className="block text-[11px] font-bold text-stone-700 mb-1">CTA Button Text & Link</label>
                        <div className="grid grid-cols-2 gap-2">
                          <input
                            type="text"
                            value={currentSlide.cta || 'Shop Now'}
                            onChange={(e) => updateCurrentSlide('cta', e.target.value)}
                            placeholder="Button Label"
                            className="w-full px-3 py-2 rounded-xl bg-stone-50 border border-stone-200 text-xs font-medium focus:bg-white focus:outline-none"
                          />
                          <input
                            type="text"
                      ...
                      <div>
                        <label className="block text-[11px] font-bold text-stone-700 mb-1">Background Photo URL</label>
                        <input
                          type="text"
                ...
                {/* 1-Click Curated Image Picker */}
                <div className="pt-3 border-t border-stone-100">
                  <span className="text-[11px] font-bold text-stone-700 block mb-2">1-Click Curated Luxury Photos:</span>
                  <div className="grid grid-cols-5 gap-2">
                    {CURATED_IMAGES.map((img, i) => (
                      <button
                        ...
                        className="aspect-square rounded-xl overflow-hidden border border-stone-200 hover:ring-2 hover:ring-stone-900 transition-all cursor-pointer"
                        title={img.title}
                      >
            ...
            {/* Other Section Inspectors */}
            {selectedSectionId && selectedSectionId !== 'hero' && (
              <div className="space-y-4">
                <div className="bg-stone-50 p-4 rounded-xl border border-stone-200 space-y-3">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-bold text-stone-900">{selectedSection?.name} Settings</span>
                    <button
                      type="button"
                      onClick={() => toggleSectionVisibility(selectedSectionId)}
                      className="px-2.5 py-1 rounded-lg text-xs font-bold bg-white border border-stone-200 text-stone-700 cursor-pointer"
                    >
                      {selectedSection?.enabled ? 'Active (Live)' : 'Hidden'}
                    </button>
                  </div>
                  <p className="text-xs text-stone-500">
                    Configure block presentation, headlines, and responsive grid layouts.
                  </p>
                </div>

                <div>
                  <label className="block text-[11px] font-bold text-stone-700 mb-1">Section Display Title</label>
                  <input
                    type="text"
            ...
            {/* ── TAB 1: SECTION NAVIGATOR & TREE (When no section is open) ── */}
            {!selectedSectionId && activeTab === 'navigator' && (
              <div className="space-y-4">
                <div className="flex items-center justify-between border-b border-stone-100 pb-3">
                  <div>
                    <h3 className="text-xs font-bold uppercase tracking-wider text-stone-900">Page Blocks Navigator</h3>
                    <p className="text-[11px] text-stone-500 mt-0.5">Drag to reorder sections. Click any section to configure.</p>
                  </div>
                </div>
            ...
            {/* ── TAB 2: GLOBAL DESIGN SYSTEM (Colors & Fonts) ── */}
                    ...
                    {CURATED_PALETTES.map((pal) => (
                      ...
                      >
                        <div className="flex items-center gap-1.5 mb-2">
                          <div className="w-3.5 h-3.5 rounded-full shadow-xs" style={{ backgroundColor: pal.primary }} />
                          <div className="w-3.5 h-3.5 rounded-full shadow-xs" style={{ backgroundColor: pal.accent }} />
                          <div className="w-3.5 h-3.5 rounded-full border border-stone-200 shadow-xs" style={{ backgroundColor: pal.background }} />
                        </div>
                        <span className="text-xs font-bold text-stone-900 block truncate">{pal.name}</span>
                      </button>
                    ))}
                  </div>
                </div>
                ...
                <div className="pt-3 border-t border-stone-100">
                  ...
                  <div className="space-y-2">
                    {CURATED_FONTS.map((font) => (
                      ...
                      >
                        <div>
                          <p className="text-xs font-bold" style={{ fontFamily: `'${font.name}', sans-serif` }}>
                            {font.name}
                          </p>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3.5 py-2.5 rounded-xl bg-stone-50 border border-stone-200 text-xs font-bold"
                  />
                </div>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full px-3.5 py-2.5 rounded-xl bg-stone-50 border border-stone-200 text-xs resize-none"
                  />
                </div>
        ...
        {/* ── Right Canvas: Live Interactive Responsive Preview ── */}
        <div className="lg:col-span-7 xl:col-span-8 flex flex-col items-center">
          {/* Canvas Bezel */}
          <div
            className={`w-full transition-all duration-300 rounded-3xl overflow-hidden border border-stone-300 shadow-xl bg-white ${
              deviceMode === 'mobile'
                ? 'max-w-[375px]'
                : deviceMode === 'tablet'
                ? 'max-w-[768px]'
          ...
          >
            ...
            <div className="bg-stone-900 text-stone-400 px-4 py-2.5 flex items-center justify-between text-xs select-none">
              <div className="flex items-center gap-1.5">
                <div className="w-2.5 h-2.5 rounded-full bg-red-500/80" />
                <div className="w-2.5 h-2.5 rounded-full bg-yellow-500/80" />
                <div className="w-2.5 h-2.5 rounded-full bg-green-500/80" />
              </div>
              <span className="font-mono text-[11px] text-stone-300 truncate max-w-[240px]">
            ...
            {/* Live Storefront Canvas Content */}
            <div
              className="max-h-[680px] overflow-y-auto scrollbar-hide text-stone-900"
              style={{
            ...
            >
              ...
              {theme.header.showAnnouncement && (
                <div
                  className="py-2 px-3 text-center text-xs font-bold tracking-wide transition-colors"
                  style={{
              ...
              {/* 2. Header */}
              <div className="bg-white/95 backdrop-blur-md border-b border-stone-200 px-5 py-3.5 flex items-center justify-between sticky top-0 z-20">
                <div className="flex items-center gap-3">
                  {storeInfo.logo_url ? (
                    <img
                    ...
                    <div
                      className="w-8 h-8 rounded-xl text-white font-bold flex items-center justify-center text-xs shadow-xs"
                ...
                <div className="flex items-center gap-2 text-xs font-semibold text-stone-600">
                  <span className="px-2.5 py-1 rounded-lg bg-stone-100">Products</span>
                  <span className="px-2.5 py-1 rounded-lg bg-stone-100">Categories</span>
                  <span className="px-3 py-1 rounded-full bg-stone-900 text-white font-bold text-xs">
                    Cart (0)
                  </span>
                </div>
              </div>

              {/* 3. Render Simulated Sections (Elementor Click-to-Edit Enabled!) */}
              {theme.sections
                .filter((s) => s.enabled)
                .map((sec) => {
                  ...
                  if (sec.id === 'hero') {
                    return (
                      ...
                      >
                        {/* Hover Elementor Pro Tag */}
                        <div className="absolute top-2 left-2 z-30 opacity-0 group-hover:opacity-100 transition-opacity bg-stone-900 text-white px-2.5 py-1 rounded-lg text-[10px] font-bold uppercase tracking-wider shadow-lg flex items-center gap-1">
                          <Edit3 className="w-3 h-3" /> Edit Hero Section
                        </div>

                        {hero.layout === 'minimal' && (
                          <div className="py-16 px-6 text-center space-y-3 bg-white">
                            {hero.topTitle && (
                            ...
                            <div className="pt-2">
                              <span
                                className="px-5 py-2 rounded-full text-white text-xs font-bold inline-flex items-center gap-1.5 shadow-xs"
                                style={{ backgroundColor: theme.palette.primary }}
                              >
                        ...
                        {hero.layout === 'immersive' && (
                          <div className="relative min-h-[260px] p-8 text-center text-white flex flex-col items-center justify-center overflow-hidden">
                            <img
                            ...
                            <div className="relative z-10 space-y-2 max-w-sm">
                              ...
                              <div className="pt-2">
                                <span className="px-5 py-2 rounded-full bg-white text-stone-900 text-xs font-bold inline-flex items-center gap-1 shadow-md">
                                  {hero.cta || 'Explore'} <ArrowRight className="w-3.5 h-3.5" />
                                </span>
                              </div>
                            </div>
                          </div>
                        ...
                        {hero.layout === 'split' && (
                          <div className="p-6 sm:p-8 grid grid-cols-2 gap-6 items-center bg-white">
                            <div className="space-y-3">
                              ...
                              <p className="text-stone-500 text-xs line-clamp-2">{hero.subtitle}</p>
                              <span
                                className="px-4 py-2 rounded-full text-white text-xs font-bold inline-flex items-center gap-1 shadow-xs"
                                style={{ backgroundColor: theme.palette.primary }}
                              >
                                {hero.cta || 'Shop Now'} <ArrowRight className="w-3.5 h-3.5" />
                              </span>
                        ...
                        {hero.layout === 'carousel' && (
                          <div className="p-6 sm:p-8 grid grid-cols-2 gap-6 items-center bg-stone-50/80 border-b border-stone-200/60">
                            <div className="space-y-3">
                              ...
                              <p className="text-stone-600 text-xs line-clamp-2">{currentSlide.subtitle}</p>
                              <span
                                className="px-5 py-2 rounded-full text-white text-xs font-bold inline-flex items-center gap-1 shadow-xs"
                                style={{ backgroundColor: theme.palette.primary }}
                              >
                                {currentSlide.cta || 'Shop Now'} <ArrowRight className="w-3.5 h-3.5" />
                              </span>
                            </div>
                            <div className="aspect-video rounded-2xl overflow-hidden bg-stone-200 border border-stone-200 shadow-sm">
                  ...
                  if (sec.id === 'features') {
                    return (
                      <div
                        key="sim-features"
                        onClick={() => setSelectedSectionId('features')}
                        onMouseEnter={() => setHoveredSectionId('features')}
                        onMouseLeave={() => setHoveredSectionId(null)}
                        className={`p-6 border-b border-stone-200/60 bg-white grid grid-cols-3 gap-4 text-center cursor-pointer relative group ${
                          isSelected ? 'ring-2 ring-stone-900 ring-offset-2' : isHovered ? 'ring-2 ring-stone-400/80' : ''
                        }`}
                      >
                        <div className="absolute top-2 left-2 z-30 opacity-0 group-hover:opacity-100 transition-opacity bg-stone-900 text-white px-2.5 py-1 rounded-lg text-[10px] font-bold uppercase tracking-wider shadow-lg flex items-center gap-1">
                          <Edit3 className="w-3 h-3" /> Edit Trust Badges
                        </div>
                        <div className="flex flex-col items-center">
                          <Truck className="w-5 h-5 text-stone-800 mb-1" />
                          <p className="text-[11px] font-bold text-stone-900">Complimentary Shipping</p>
                          <p className="text-[10px] text-stone-500">On all orders above ₹499</p>
                        </div>
                        <div className="flex flex-col items-center">
                          <ShieldCheck className="w-5 h-5 text-stone-800 mb-1" />
                          <p className="text-[11px] font-bold text-stone-900">100% Authentic</p>
                          <p className="text-[10px] text-stone-500">Certified quality guarantee</p>
                        </div>
                        <div className="flex flex-col items-center">
                          <Headphones className="w-5 h-5 text-stone-800 mb-1" />
                          <p className="text-[11px] font-bold text-stone-900">Concierge Support</p>
                          <p className="text-[10px] text-stone-500">Dedicated assistance 24/7</p>
                        </div>
                      </div>
                    );
                  }
                  ...
                  if (sec.id === 'categories') {
                    return (
                      <div
                        ...
                        onMouseLeave={() => setHoveredSectionId(null)}
                        className={`p-6 border-b border-stone-200/60 bg-white space-y-4 cursor-pointer relative group ${
                          isSelected ? 'ring-2 ring-stone-900 ring-offset-2' : isHovered ? 'ring-2 ring-stone-400/80' : ''
                        }`}
                      >
                        <div className="absolute top-2 left-2 z-30 opacity-0 group-hover:opacity-100 transition-opacity bg-stone-900 text-white px-2.5 py-1 rounded-lg text-[10px] font-bold uppercase tracking-wider shadow-lg flex items-center gap-1">
                          <Edit3 className="w-3 h-3" /> Edit Categories Grid
                        </div>
                        <div className="flex items-center justify-between">
                          <h3 className="text-sm font-black text-stone-900 uppercase tracking-wider">Curated Collections</h3>
                        ...
                        <div className="grid grid-cols-4 gap-3">
                          {['Attar Oils', 'Eau de Parfum', 'Home Scents', 'Gift Sets'].map((cat, i) => (
                            <div key={i} className="rounded-xl overflow-hidden border border-stone-200 bg-stone-50 p-2 text-center space-y-1">
                              <div className="aspect-square rounded-lg bg-stone-200 overflow-hidden">
                                <img src={CURATED_IMAGES[i % CURATED_IMAGES.length].url} alt={cat} className="w-full h-full object-cover" />
                              </div>
                              <p className="text-[10px] font-bold text-stone-800 truncate">{cat}</p>
                            </div>
                          ))}
                        </div>
                      </div>
                  ...
                  if (sec.id === 'featured_products' || sec.id === 'latest_arrivals') {
                    return (
                      ...
                      >
                        <div className="absolute top-2 left-2 z-30 opacity-0 group-hover:opacity-100 transition-opacity bg-stone-900 text-white px-2.5 py-1 rounded-lg text-[10px] font-bold uppercase tracking-wider shadow-lg flex items-center gap-1">
                          <Edit3 className="w-3 h-3" /> Edit {sec.name}
                        </div>
                        <div className="flex items-center justify-between">
                          <h3 className="text-sm font-black text-stone-900 uppercase tracking-wider">{sec.name}</h3>
                          <span className="text-[11px] font-bold text-stone-500">Explore Catalog</span>
                        </div>
                        <div className="grid grid-cols-3 gap-3">
                          {[
                            ...
                            <div key={i} className="rounded-xl border border-stone-200 bg-white p-2 space-y-1.5 shadow-xs">
                              ...
                              <p className="text-[11px] font-black text-stone-900">{item.price}</p>
                            </div>
                          ))}
                        </div>
                      </div>
                  ...
                  if (sec.id === 'promo_banner') {
                    return (
                      <div
                        key="sim-promo"
                        onClick={() => setSelectedSectionId('promo_banner')}
                        onMouseEnter={() => setHoveredSectionId('promo_banner')}
                        onMouseLeave={() => setHoveredSectionId(null)}
                        className={`p-8 bg-stone-900 text-white text-center space-y-2 cursor-pointer relative group ${
                          isSelected ? 'ring-2 ring-stone-900 ring-offset-2' : isHovered ? 'ring-2 ring-stone-400/80' : ''
                        }`}
                      >
                        <div className="absolute top-2 left-2 z-30 opacity-0 group-hover:opacity-100 transition-opacity bg-white text-stone-900 px-2.5 py-1 rounded-lg text-[10px] font-bold uppercase tracking-wider shadow-lg flex items-center gap-1">
                          <Edit3 className="w-3 h-3" /> Edit Promo Banner
                        </div>
                        <span className="px-2.5 py-0.5 rounded-full bg-white/20 text-white text-[9px] font-bold uppercase tracking-widest inline-block">
                          Exclusive Offer
                        </span>
                        <h3 className="text-base font-black">20% Off Your First Purchase</h3>
                        <p className="text-xs text-stone-300">Use code PRIVILEGE at checkout for instant luxury savings.</p>
                      </div>
                    );
                  }
                  ...
                  if (sec.id === 'newsletter') {
                    return (
                      <div
                        key="sim-newsletter"
                        onClick={() => setSelectedSectionId('newsletter')}
                        onMouseEnter={() => setHoveredSectionId('newsletter')}
                        onMouseLeave={() => setHoveredSectionId(null)}
                        className={`p-6 bg-stone-100 text-center space-y-2 cursor-pointer relative group ${
                          isSelected ? 'ring-2 ring-stone-900 ring-offset-2' : isHovered ? 'ring-2 ring-stone-400/80' : ''
                        }`}
                      >
                        <div className="absolute top-2 left-2 z-30 opacity-0 group-hover:opacity-100 transition-opacity bg-stone-900 text-white px-2.5 py-1 rounded-lg text-[10px] font-bold uppercase tracking-wider shadow-lg flex items-center gap-1">
                          <Edit3 className="w-3 h-3" /> Edit Newsletter
                        </div>
                        <h3 className="text-sm font-black text-stone-900">Join the Private Atelier Club</h3>
                        <p className="text-xs text-stone-500">Receive private release announcements & bespoke offers.</p>
                        <div className="max-w-xs mx-auto flex gap-1.5 pt-1">
                          <input
                            type="text"
              ...
              {/* 4. Footer */}
              <div className="bg-white border-t border-stone-200 p-6 space-y-4 text-xs text-stone-500">
                <div className="flex items-center justify-between">
