...
import { useCustomerNotifications, NotificationPreferences } from '@/shared/hooks/customer/useCustomerNotifications';

export const NotificationsPage: React.FC = () => {
  const { preferences: initialPrefs, isLoading: loading, updatePreferences, isUpdating: saving } = useCustomerNotifications();
  const [preferences, setPreferences] = useState<NotificationPreferences | null>(null);
  
  // Update local state when remote data loads
  useEffect(() => {
    if (initialPrefs && !preferences) {
      setPreferences(initialPrefs);
    }
  }, [initialPrefs, preferences]);

  const hasChanges = preferences && initialPrefs && JSON.stringify(preferences) !== JSON.stringify(initialPrefs);
    ...
    return (
      <CustomerDashboardLayout title="Notifications" subtitle="Manage your notification preferences">
        <div className="max-w-2xl space-y-4">
          {[...Array(3)].map((_, i) => (
            <div key={i} className="bg-white rounded-2xl p-6 hidden">
