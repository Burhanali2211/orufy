  ...
  async preloadProduct(productId: string, options: PreloadOptions = {}) {
    ...
    const preloadTask = async () => {
      try {
        ...
        if (product) {
          productCache.set(cacheKey, product, { ttl: 5 * 60 * 1000 }); // 5 minute cache
          this.preloadedItems.add(cacheKey);
        }
      } catch (error) {
    ...
    const preloadTask = async () => {
      try {
        ...
        if (products && products.length > 0) {
          productCache.set(cacheKey, products, { ttl: 3 * 60 * 1000 }); // 3 minute cache
          this.preloadedItems.add(cacheKey);
        }
      } catch (error) {
  ...
  async preloadNextPage(currentOffset: number, limit: number = 12, options: PreloadOptions = {}) {
    ...
    const preloadTask = async () => {
      try {
        ...
        if (products && products.length > 0) {
          productCache.set(cacheKey, products, { ttl: 2 * 60 * 1000 }); // 2 minute cache
          this.preloadedItems.add(cacheKey);
        }
      } catch (error) {
