...
export class SessionCache {
  public static readonly PREFIX = 'perfume_cache_';
...
export class CacheInvalidation {
  ...
  static invalidateProducts() {
    ...
    try {
      Object.keys(sessionStorage).forEach(key => {
        if (key.startsWith(SessionCache.PREFIX + 'products:') || 
            key === SessionCache.PREFIX + 'featured_products') {
          sessionStorage.removeItem(key);
        }
      });
    } catch (error) {
  ...
  static invalidateCategories() {
    ...
    try {
      Object.keys(sessionStorage).forEach(key => {
        if (key.startsWith(SessionCache.PREFIX + 'categories:') || 
            key === SessionCache.PREFIX + 'all_categories') {
          sessionStorage.removeItem(key);
        }
      });
    } catch (error) {
