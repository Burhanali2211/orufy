...
import { useMobileDetection } from '@/shared/hooks/useMobileGestures';
    ...
    return (
      <div className={`${className}`}>
        <div className="flex items-center space-x-2 mb-3">
          ...
          <div>
            <div className="h-4 bg-neutral-200 rounded w-32 hidden"></div>
            <div className="h-3 bg-neutral-100 rounded w-36 mt-1 hidden"></div>
          </div>
        </div>
        <div className={`grid gap-3 ${layout === 'grid' ? 'grid-cols-2 sm:grid-cols-3 md:grid-cols-4' : 'grid-cols-1 sm:grid-cols-2 md:grid-cols-3'}`}>
          {Array.from({ length: maxItems }).map((_, index) => (
            <div key={index} className="bg-white rounded-lg border border-neutral-200 p-3 hidden">
