...
export const ProductCard: React.FC<ProductCardProps> = memo(({
  ...
  const { isTech, isFashion } = useMemo(() => {
    const catName = (product.category || (product as any).categoryName || '').toLowerCase();
    return {
  ...
  if (isListView) {
    return (
      <div className="group flex flex-row gap-3 sm:gap-6 p-2.5 sm:p-4 bg-white rounded-xl sm:rounded-2xl border border-stone-200 hover:border-stone-400 transition-all duration-300 shadow-xs hover:shadow-md relative">
        {/* Image Section - Fixed width on all screens */}
        <div className="relative w-28 sm:w-40 md:w-52 lg:w-64 flex-shrink-0 overflow-hidden rounded-xl bg-stone-50 border border-stone-200">
        ...
        <div className="flex flex-col flex-1 min-w-0 justify-between py-0 sm:py-2">
          <div>
            ...
            <div className="flex items-center gap-2 mb-1 sm:mb-2">
              <span className="text-[8px] sm:text-[10px] font-bold uppercase tracking-wider px-1.5 sm:px-2 py-0.5 rounded bg-stone-100 text-stone-800 border border-stone-200 truncate max-w-[80px] sm:max-w-none">
                {product.category || (product as any).categoryName || 'Discovery'}
              </span>
              <div className="flex items-center text-amber-500 ml-auto flex-shrink-0">
                <Star className="h-3 sm:h-3.5 w-3 sm:w-3.5 fill-current" />
                <span className="text-xs sm:text-sm font-bold text-stone-900 ml-0.5 sm:ml-1">{product.rating || '4.5'}</span>
                <span className="text-[10px] sm:text-xs text-stone-400 ml-0.5 sm:ml-1 font-medium hidden sm:inline">(2.4k)</span>
              </div>
            </div>
            
            {/* Product Name */}
            <Link to={`/products/${product.id}`}>
              <h3 className="text-sm sm:text-lg md:text-xl font-bold font-serif text-stone-900 mb-1 sm:mb-2 group-hover:text-stone-700 transition-colors line-clamp-1">
                {product.name}
              </h3>
            </Link>
            
            {/* Description - Hidden on very small screens */}
            <p className="hidden sm:block text-xs sm:text-sm text-stone-600 line-clamp-2 leading-relaxed mb-2 sm:mb-4 font-normal">
              {product.shortDescription || product.description}
            </p>
            
            {/* Trust Badges - Simplified on mobile */}
            <div className="hidden md:flex flex-wrap items-center gap-4 text-[11px] text-stone-600 font-medium">
              <span className="flex items-center gap-1.5"><Check className="h-3.5 w-3.5 text-stone-800" /> Free Returns</span>
              <span className="flex items-center gap-1.5"><Zap className="h-3.5 w-3.5 text-stone-800" /> Get it by <b>Tomorrow</b></span>
              <span className="flex items-center gap-1.5"><Check className="h-3.5 w-3.5 text-stone-800" /> Cash on Delivery</span>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between mt-2 sm:mt-4 gap-2 sm:gap-4">
            <div className="flex flex-col sm:flex-row sm:items-baseline gap-0 sm:gap-2">
              <span className="text-lg sm:text-2xl md:text-3xl font-black text-[#131921]">₹{(product.price / 100).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
              {product.originalPrice && (
                <span className="text-[10px] sm:text-sm text-gray-400 line-through font-medium">₹{(product.originalPrice / 100).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
              )}
            </div>
