...
const INDIAN_STATES = [
  ...
  'Telangana', 'Tripura', 'Uttar Pradesh', 'Jammu and Kashmir', 'West Bengal',
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        ...
        >
          ...
          <div className="text-right">
            <div className="text-lg font-bold text-gray-900">₹{(total / 100).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</div>
          ...
          <div className="border-t border-gray-100 p-5 space-y-4">
              ...
              {items.map((item) => (
                <div key={item.id || item.product?.id} className="flex items-center gap-3">
                  ...
                  <div className="text-sm font-semibold text-gray-900">
                    ₹{item.product?.price ? (Number(item.product.price) * item.quantity / 100).toLocaleString('en-IN', { minimumFractionDigits: 2 }) : '0'}
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="border-t border-gray-100 pt-4 space-y-2">
              <div className="flex justify-between text-sm">
                ...
                <span className="font-medium">₹{(subtotal / 100).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-gray-600">Shipping</span>
                <span className={`font-medium ${shipping === 0 ? 'text-green-600' : ''}`}>
                  {shipping === 0 ? 'FREE' : `₹${(shipping / 100).toLocaleString('en-IN', { minimumFractionDigits: 2 })}`}
                </span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-gray-600">GST (18%)</span>
                <span className="font-medium">₹{(tax / 100).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
              </div>
              <div className="flex justify-between font-bold text-base border-t border-gray-100 pt-3">
                <span className="text-gray-900">Total</span>
                <span className="text-purple-600">₹{(total / 100).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
              </div>
            </div>
          </div>
        </motion.div>
